...
import 'package:rosetta_fluter_app/rosetta/models/ForceUpdate.dart';
...
import '../../rosetta/models/DashboardData.dart';
...
class ShHomeFragmentState extends State<DisHomeScreen> {
  ...
  // Dashboard Data
  int credit_limit=0;
  int available_credit_limit=0;
  int credit_used=0;
  double credit_used_per=0;
  int balance=0;
  int overdue_balance=0;
  int  purchase_orders = 0;
  int purchase_orders_amount = 0;
  //(credit_limit/credit_used)*100).toInt()


  // for show amount with commas 21,536,453
  RegExp reg = RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))');
  String Function(Match) mathFunc = (Match match) => '${match[1]},';

  // for info icons click
  bool info_outstandiang = false;
  bool info_overdue = false;
  ...
  Future<List<SpinnerSpinnerDealerList>> getDealerList() async {
    ...
    setState(() {
      ...
      spinnerDelerList = spinnerSpinnerDealerListFromJson((response.body.toString()));
      if(responseCode==200)
      {
  ...
  {
    print("Api Dashboard Data Methode Call >>>>>>>>>>>>>>>>>> ");

    http.Response response = await http.get(Uri.parse(API_ROS_PROD__BASE_URL+'/api/dashboard'),
      headers: {
        HttpHeaders.authorizationHeader: 'bearer $MyToken',
      },
    );
    var responcebody = response.body;
    var responseCode = response.statusCode;
    print("Response Body of Form SpinnerDealer List Methode   Dashboard Data Api : $responcebody");
    print("Response code Form SpinnerDealer List Methode  Dashboard Data Api : $responseCode");

    if(responseCode==200)
    {
      DashBoardData obj =   DashBoardData.fromJson(json.decode(responcebody));
      List<Tile> tiles = obj.tiles;
      List<Monthly> monthly = obj.monthly;

      print("List of Monthly Data "+monthly.toString());
      print("List of Tiles Data "+tiles.toString());


      setState(() {
        if(tiles.length>=1) {
          credit_limit = tiles[0].value.toString().toDouble().floor();
          available_credit_limit = tiles[1].value.toString().toDouble().floor();

        }
        if(tiles.length>=3)
        {
          balance = tiles[2].value.toString().toDouble().floor();
        }
        if(tiles.length>=4)
        {
          overdue_balance = tiles[3].value.toString().toDouble().floor();
          print("Dashboard Data 1 " + tiles[0].value);
        }
      ...
      setState(() {
        if(monthly.length>=4) {
          purchase_orders = monthly[0].value.toDouble().floor();
          purchase_orders_amount = monthly[1].value.toDouble().floor();
          // balance = tiles[2].value.toDouble().floor();
          // overdue_balance = tiles[3].value.toDouble().floor();
          // print("Dashboard Data 1 " + tiles[0].value);
        }
      ...
      print("Dashboard Data limits $credit_limit");
      print("Dashboard Data available limits  $available_credit_limit");
      print("Dashboard Data balance $balance");
      print("Dashboard Data overdue_balance $overdue_balance");
      print("Dashboard Data purchase_orders $purchase_orders");
      print("Dashboard Data purchase_orders_amount $purchase_orders_amount");

      // int per = ((available_credit_limit/credit_limit)*100).toInt();
      // print("Dashboard Data limits Percentage $per");

      //(available_credit_limit/credit_limit)

      setState(() {

        credit_used =   credit_limit-available_credit_limit;
        print("On Per "+credit_limit.toString());
        double value = credit_limit/credit_used;
        print("Fraction value "+value.toString());
        credit_used_per =((credit_used/(credit_limit))*100);

      });

      print("credit Used  >>>>>>>>>>>>>>>>>>>  "+credit_used.toString());
      print("credit Per  >>>>>>>>>>>>>>>>>>>  "+credit_used_per.toStringAsFixed(2));
  ...
  String getCurrentMonth()
  {
    ...
    return months[current_mon-1]+" "+formatted;
  }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Get Dealer List Refined
  ...
  getList() async {
    ...
    print("Api Methode Calll For CMS Collections ++++++++++++ IS CMS ++++++++++++ ");
    ...
    try {
      ...
      print("LENGTH TTTTTTTTTTTTT  CMS FIRST ");
  ...
  getMainCollection() async{
    print(" Get Main Collection  Api Mehtode Call  +++++++++++ IS ROS ++++++++++++++");
    ...
    print(response.body.length);
    print("HOME TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT");
    setState(()
    {
  ...
  getForceUpdate(String appversionNo) async {
    print("Force update  Api Mehtode Call ");
    var map = new Map<String, dynamic>();
    if(Platform.isAndroid)
    {
      print("Force update Android ");
      map['device_type'] = "android";
    }
    if(Platform.isIOS)
    {
      print("Force update Ios ");
      map['device_type'] = "ios";
    }
    ...
    http.Response response;
    try{
      response = await http.post(Uri.parse(API_CMS_BASE_URL+'/api/webservices/checkVersion'), body: map,);
    ...
    var responcebody = response.body;
    var responseCode = response.statusCode;
    print("Response Body of Form Force Updtae Data Api : $responcebody");
    print("Response code Form Main Force Update  Data Api : $responseCode");
    ...
    if(responseCode==200)
    {
      ...
      var responseCode = response.statusCode;
      print("Response Body of Form Force Updtae Data Api : $responcebody");
      print("Response code Form Main Force Updtae  Data Api : $responseCode");

      ForceUpdate obj =  ForceUpdate.fromJson(json.decode(responcebody));

      bool forceUpdate = obj.status;
      double updatedVersion  = double.parse(obj.appVersion);

      if(forceUpdate)
      {
        double oldVersionNo = double.parse(appversionNo);

        if(updatedVersion>oldVersionNo)
        {
          print("Force Update Active");
          showForceUpdateDialog(context,obj.forceUpdate);
  ...
  getNewProducts() async{
    print("Get New Prioduct Api  Mehtode Call ++++ CMS ++++++");
...
//     ].request();
  ...
  getSessionData() async
  {
    ...
    if(token.isNotEmpty)
    {
      ...
      if(checkNetconnection!=null)
      {
        ...
        String AppVersionNo =   version.substring(0,4);
        ...
        print("AppVersionNo $AppVersionNo");
    ...
    print("myTimeStamp >>>>>>>>>>>>>>>>>>> "+prefs2.getString("myTimeStamp").toString());
    print("cuurentTimeStamp >>>>>>>>>>>>>>>>>>> "+cuurentTimeStamp);
    bool isShowDialog = false;
    if(cuurentTimeStamp!=prefs2.getString("myTimeStamp")||prefs2.getString("myTimeStamp").isEmptyOrNull)
    {
    ...
    prefs2.setString("myTimeStamp", cuurentTimeStamp);
    print("myTimeStamp 2"+prefs.getString("myTimeStamp").toString());
                  ...
                  MediaQuery(
                    ...
                    Scaffold(
                      ...
                      appBar: PreferredSize(
                        ...
                        child:MyAppBar(true,"Home",MyUserName+" @",isVisibleSkip,true),
                          ...
                          child:SingleChildScrollView(
                              ...
                              child: ConstrainedBox(
                                ...
                                child: Column(
                                  ...
                                  children: <Widget>[
                                    true?Container():StringNoticeMessage.isEmpty?Container():  Container(
                                              ...
                                              fieldViewBuilder: (BuildContext context,
                                                ...
                                                return TextFormField(
                                                  ...
                                                  decoration: InputDecoration(
                                                    ...
                                                    hintText:isDealerAvailbe? "Search Dealers":"No Dealers Available",
                                    ...
                                    //     Stack(
                                    //       children: [
                                    //         available_credit_limit==0||available_credit_limit==1?Container(
                                    //           width: 150,
                                    //
                                    //         ):
                                    //         CircularStepProgressIndicator(
                                    //           //totalSteps: available_credit_limit=="1"?100:double.parse(available_credit_limit).floor(),
                                    //           //currentStep: credit_limit=="1"?1:double.parse(credit_limit).floor(),
                                    //           totalSteps:100,
                                    //           currentStep:((available_credit_limit/credit_limit)*100).toInt(),
                                    //           stepSize: 10,
                                    //           selectedColor: sh_colorAvaiableCredit,
                                    //           unselectedColor: sh_colorTotalCredit,
                                    //           padding: 0,
                                    //           width: 150,
                                    //           height: 150,
                                    //           selectedStepSize: 10,
                                    ...
                                    //             width: 150,
                                    //             height: 150,
                                    //             child:  Center(child: Text("Credit",style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorAvaiableCredit,fontWeight: FontWeight.bold ),)),
                                    //         )
                                    //
                                    //       ],
                                    //     ),
                                    ...
                                    //     Stack(
                                    //       children: [
                                    //         balance==0||balance==1?Container(
                                    //           width: 150,
                                    //
                                    //         ):CircularStepProgressIndicator(
                                    //           totalSteps: 100,
                                    //           currentStep: ( (overdue_balance/balance)*100).toInt(),
                                    //           stepSize: 10,
                                    //           selectedColor: sh_colorOverdue,
                                    //           unselectedColor: sh_colorBalance,
                                    //           padding: 0,
                                    //           width: 150,
                                    //           height: 150,
                                    //           selectedStepSize: 10,
                                    ...
                                    //           roundedCap: (_, __) => false,
                                    //         ),
                                    //         balance==0||balance==1?Container(): Container(
                                    //           width: 150,
                                    //           height: 150,
                                    //           child:  Center(child: Text("Balance",style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorBalance,fontWeight: FontWeight.bold ),)),
                                    //         )
                                    //       ],
                                    //     ),
                                    //
                                    ...
                                    //             Row(
                                    //               children: [
                                    //                 Image.asset(ic_round_color,height: 12,width: 12, fit: BoxFit.fill,color: sh_colorAvaiableCredit,),
                                    //                 SizedBox(width: 10,),
                                    //                 Text("Limit",style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary, ),),
                                    //               ],
                                    //             ),
                                    //           SizedBox(width: 10,),
                                    //
                                    //           Text("$credit_limit".toCurrencyFormat(),style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
                                    //
                                    //
                                    //
                                    //          ],
                                    ...
                                    //             Row(
                                    //               children: [
                                    //                 Image.asset(ic_round_color,height: 12,width: 12, fit: BoxFit.fill,color: sh_colorBalance,),
                                    //                 SizedBox(width: 10,),
                                    //                 Text("Balance",style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary, ),),
                                    //               ],
                                    //             ),
                                    //             SizedBox(width: 10,),
                                    //
                                    //
                                    //
                                    //             Text("$balance".toCurrencyFormat(), overflow: TextOverflow.ellipsis,style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
                                    //
                                    //
                                    //
                                    //           ],
                                    ...
                                    //             Row(
                                    //               children: [
                                    //                 Image.asset(ic_round_color,height: 12,width: 12, fit: BoxFit.fill,color: sh_colorTotalCredit,),
                                    //                 SizedBox(width: 10,),
                                    //                 Text("Total",style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary, ),),
                                    //               ],
                                    //             ),
                                    //             SizedBox(width: 10,),
                                    //             Text("$available_credit_limit".toCurrencyFormat(),style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
                                    //
                                    //
                                    //
                                    //           ],
                                    ...
                                    //             Row(
                                    //               children: [
                                    //                 Image.asset(ic_round_color,height: 12,width: 12, fit: BoxFit.fill,color: sh_colorOverdue,),
                                    //                 SizedBox(width: 10,),
                                    //                 Text("Overdue",style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary, ),),
                                    //               ],
                                    //             ),
                                    //             SizedBox(width: 10,),
                                    //             Text("$overdue_balance".toCurrencyFormat(),style: TextStyle(fontSize: textSizeSMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
                                    //
                                    //
                                    //
                                    //           ],
                                            ...
                                            children: [
                                              ...
                                              Container(
                                                ...
                                                Column(
                                                  mainAxisAlignment: MainAxisAlignment.start,
                                                  children: [
                                                    Text("Credit Limit",style: TextStyle(fontSize: textSizeSMedium,color:Colors.black, ),),
                                                    Text(credit_limit.toString().replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorAvaiableCredit,fontWeight: FontWeight.bold ),)
                                        ...
                                        child:  Stack(
                                          children: [
                                            ...
                                            CircularStepProgressIndicator(
                                              ...
                                              //currentStep: credit_limit=="1"?1:double.parse(credit_limit).floor(),
                                              totalSteps:100,
                                              //currentStep:((60/100)*100).toInt(),
                                              currentStep:credit_used_per.toInt(),
                                              selectedColor: sh_colorAvaiableCredit,
                                              unselectedColor: sh_colorTotalCredit,
                                                    ...
                                                    children: [
                                                      ...
                                                      Container(
                                                        ...
                                                        Column(
                                                          ...
                                                          children: [
                                                            //credit_used_per
                                                            Text(credit_used_per.toStringAsFixed(2)+"%",style: TextStyle(fontSize: textSizeLarge2,color:sh_colorAvaiableCredit,fontWeight: FontWeight.bold ),),
                                                            Text("Credit Used",style: TextStyle(fontSize: textSizeMedium,color:Colors.black,fontWeight: FontWeight.bold ),)
                                                ...
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Image.asset(ic_credit_used,height: 15,width: 15, fit: BoxFit.fill,color: sh_colorPrimary,),
                                                      Column(
                                                        mainAxisAlignment: MainAxisAlignment.start,
                                                        crossAxisAlignment: CrossAxisAlignment.start,

                                                        children: [
                                                          Container(margin: EdgeInsets.only(left: 10), child: Text("Credit Used",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, fontWeight: FontWeight.w500 ),)),
                                                          SizedBox(height: 5,),

                                                          //available_credit_limit/credit_limit

                                                          Container(margin: EdgeInsets.only(left: 10), child: Text(credit_used.toString().replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorPrimary,fontWeight: FontWeight.bold ),)
                                                            //Container(margin: EdgeInsets.only(left: 10), child: Text(credit_limit.toString().replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorPrimary,fontWeight: FontWeight.bold ),)
                                                ...
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Image.asset(ic_credit_available,height: 15,width: 15, fit: BoxFit.fill,),
                                                      Column(
                                                        mainAxisAlignment: MainAxisAlignment.start,
                                                        crossAxisAlignment: CrossAxisAlignment.start,

                                                        children: [
                                                          Container(margin: EdgeInsets.only(left: 10), child: Text("Credit Available",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, fontWeight: FontWeight.w500),)),
                                                          SizedBox(height: 5,),
                                                          Container(margin: EdgeInsets.only(left: 10), child: Text(available_credit_limit.toString().replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:Colors.green,fontWeight: FontWeight.bold ),)
                                          ...
                                          Container(
                                              ...
                                              child: Stack(
                                                children: [
                                                  ...
                                                  Image.asset(ic_bg_info_arrow, color: sh_colorPrimary,),
                                                  Container(

                                                      padding: EdgeInsets.all(5.0),
                                                      child: Text("The total amount of all the sales invoices that are unpaid \nNote :- this amount includes the overdue bills also ",style: TextStyle(fontSize: textSizeVerySmall,color:Colors.white, ),)),
                                                ],
                                              )
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Text("Total Outstanding",style: TextStyle(fontSize: textSizeLargeMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),),
                                              ...
                                              InkWell(
                                                onTap: (){
                                                  setState(() {
                                                    ...
                                                    if(info_outstandiang)
                                                    {
                                                      info_outstandiang=false;
                                                    }
                                                    else
                                                    {
                                                      info_outstandiang=true;

                                                    }
                                                  });
                                                },
                                                child : Image.asset(ic_info,height: 25,width: 25, fit: BoxFit.fill,),
                                              ),
                                          ...
                                          Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Text("$balance".replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_colorPrimary,fontWeight: FontWeight.bold ),),
                                              ...
                                              child: Stack(
                                                children: [
                                                  ...
                                                  Container(
                                                      ...
                                                      padding: EdgeInsets.all(5.0),
                                                      child: Text("Those bills that have passed their Due dates and is included in the Total outstanding Amount",style: TextStyle(fontSize: textSizeVerySmall,color:Colors.white, ),)),
                                                ],
                                              )
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Text("Overdue",style: TextStyle(fontSize: textSizeLargeMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),),
                                          ...
                                          Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Text("$overdue_balance".replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:Colors.red,fontWeight: FontWeight.bold ),),

                                              Text("Due Today",style: TextStyle(fontSize: textSizeLargeMedium,color:Colors.red,fontWeight: FontWeight.bold ),),
                                    ...
                                    SizedBox(height: 10,),
                                    overdue_balance.toString()=="0"?Container():   Container(
                                        margin: EdgeInsets.only(left: 10),
                                        child: Text("*Please clear overdue amount to place further orders",style: TextStyle(fontSize: textSizeSMedium,color:Colors.red, ),)),
                                    SizedBox(height: 20,),
                                    Container(
                                        padding: EdgeInsets.only(left: 10,right: 20),

                                        child: Text(getCurrentMonth()+" Order Details",style: TextStyle(fontSize: textSizeLargeMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)),
                                              ...
                                              children: [
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    Image.asset(ic_cart_dashboard,height: 25,width: 25, fit: BoxFit.fill,color:sh_colorPrimary,),
                                                    Column(
                                                      mainAxisAlignment: MainAxisAlignment.start,
                                                      crossAxisAlignment: CrossAxisAlignment.start,

                                                      children: [
                                                        Container(margin: EdgeInsets.only(left: 10), child: Text("Purchase",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, ),)),
                                                        Container(margin: EdgeInsets.only(left: 10), child: Text("Orders ",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, ),)),
                                                      ],
                                                    ),
                                                ...
                                                SizedBox(height: 10,),
                                                Text("$purchase_orders".replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
                                                ...
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Image.asset(ic_purchase_amount,height: 25,width: 25, fit: BoxFit.fill,color: sh_colorPrimary,),
                                                      Column(
                                                        mainAxisAlignment: MainAxisAlignment.start,
                                                        crossAxisAlignment: CrossAxisAlignment.start,
                                                        ...
                                                        children: [
                                                          Container(margin: EdgeInsets.only(left: 10), child: Text("Purchase",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, ),)),
                                                          Container(margin: EdgeInsets.only(left: 10), child: Text("Orders Amount",style: TextStyle(fontSize: textSizeMedium,color:sh_textColorPrimary, ),)),
                                                        ],
                                                      ),
                                                  ...
                                                  SizedBox(height: 10,),
                                                  Text("$purchase_orders_amount".toCurrencyFormat().replaceAllMapped(reg, mathFunc),style: TextStyle(fontSize: textSizeLargeMedium,color:sh_textColorPrimary,fontWeight: FontWeight.bold ),)
  ...
  showForceUpdateDialog(BuildContext contex,String  forceUpdateStatus) {
    // set up the button
...
_launchURL() async {
  ...
  String url = 'https://play.google.com/store/apps/details?id=com.rosetta.rosetta_fluter_app';
